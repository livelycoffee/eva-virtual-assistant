...
def train_task_exec(OUTPUT_FILE):
...
def main_task_exec(query):
    task = str(query).lower()
    #newtask = str(query).lower()

    return_data = tasks_exec(task)
    try:
        return_data = json.loads(str(return_data))
        run = request_exec(return_data)
        if run == True:
            return True
        else:
            return False
